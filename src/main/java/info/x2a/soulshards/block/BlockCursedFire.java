...
public class BlockCursedFire extends SoulFireBlock {
    public BlockCursedFire() {
        super(Properties.ofFullCopy(Blocks.SOUL_FIRE).mapColor(MapColor.COLOR_LIGHT_GRAY));
    }
    ...
    public void entityInside(BlockState blockState, Level level, BlockPos blockPos, Entity entity) {
        if (entity instanceof ItemEntity ent) {
            var recipes = level.getRecipeManager()
                    .getAllRecipesFor(RegistrarSoulShards.CURSING_RECIPE)
                    .stream()
                    .filter(r -> r.value().input() == ent.getItem().getItem())
                    .toList();
            if (!recipes.isEmpty()) {
                var recipeIdx = 0;
                ...
                while (!stack.isEmpty()) {
                    ...
                    stack.shrink(1);
                    var newStack = recipe.value().getResult();
